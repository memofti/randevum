...
function statusBadge(s, isDark) {
  const map = {
    confirmed:{ l:'✓ Onaylı',   cls:isDark?'bg-green-500/10 text-green-400 border-green-500/30':'bg-green-50 text-green-700 border-green-200' },
    pending:  { l:'⏳ Bekliyor', cls:isDark?'bg-amber-500/10 text-amber-400 border-amber-500/30':'bg-amber-50 text-amber-700 border-amber-200' },
    completed:{ l:'Tamamlandı',  cls:isDark?'bg-white/5 text-white/60 border-white/10':'bg-gray-100 text-gray-600 border-gray-200' },
    cancelled:{ l:'İptal',       cls:isDark?'bg-red-500/10 text-red-400 border-red-500/30':'bg-red-50 text-red-600 border-red-200' },
  }
...
export default function AppointmentsTab({
    ...
    return (
      <div className={'max-w-2xl mx-auto px-4 sm:px-6 py-8 '+(isDark?'text-white':'')}>
        <h1 className={'font-extrabold mb-6 '+(variant==='luxury'?'text-3xl tracking-widest':'text-2xl')} style={isDark?{color:accent}:{}}>
          {variant==='luxury'?'RANDEVULARIM':'Randevularım'}
        </h1>
        <div className="text-center py-16">
          <div className="text-5xl mb-4">📅</div>
          <div className="font-bold mb-4" style={{color:muted}}>Henüz randevu yok</div>
  ...
  const Section = ({ title, items, isPast }) => items.length > 0 && (
    <div className="mb-7">
      ...
      <div className="space-y-3">
        {items.map(a => {
          const b = statusBadge(a.status, isDark)
          return (
  ...
  return (
    <div className={'max-w-2xl mx-auto px-4 sm:px-6 py-8 '+(isDark?'text-white':'')}>
      ...
      <Section title="Yaklaşan" items={upcomingAppts||[]} isPast={false} />
      <Section title="Geçmiş" items={pastAppts||[]} isPast={true} />
    </div>
  )
}

...
function StatusBdg({ s }) {
  const m = {
    active:    ['bg-green-50 text-green-700 border-green-200','● Aktif'],
    review:    ['bg-amber-50 text-amber-700 border-amber-200','● İnceleme'],
    suspended: ['bg-red-50 text-red-600 border-red-200','● Askıya Alındı'],
    passive:   ['bg-gray-100 text-gray-600 border-gray-200','● Pasif'],
  }
...
function PlanBdg({ p }) {
  ...
  return <span className={`inline-flex items-center text-xs font-bold px-2 py-0.5 rounded-full border ${m[p]||m.free}`}>
    {p==='pro'?'Pro':p==='enterprise'?'Enterprise':'Ücretsiz'}
  </span>
}
...
function KPI({ label, value, sub, accent }) {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm relative overflow-hidden">
      ...
      <div className="text-3xl font-extrabold tracking-tight mb-1" style={{ color: ac[accent]||'#111' }}>{value}</div>
      {sub && <div className="text-xs font-semibold text-green-600">{sub}</div>}
      <div className="absolute bottom-0 left-0 right-0 h-0.5" style={{ background: ac[accent]||'#ccc' }} />
    </div>
  )
}
...
export default function AdminPage() {
  ...
  async function rejectFirm(id, name) {
    try {
      const { error } = await supabase.from('businesses').update({ status: 'passive' }).eq('id', id)
      if (error) throw error
  ...
  async function saveFirm() {
    ...
    try {
      ...
      const { error } = await supabase.from('businesses').insert({
        ...form, status: 'review', rating: 4.5, review_count: 0, emoji: emojis[form.category] || '🏢'
      })
      if (error) throw error
      setModal(false)
    ...
    <div className="flex h-screen overflow-hidden bg-gray-50">
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4"
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl">
            <div className="p-5 border-b border-gray-100 flex justify-between items-start">
              <div><div className="font-bold">Yeni Firma Ekle</div><div className="text-xs text-gray-500 mt-0.5">Supabase'e kaydedilecek</div></div>
            ...
            <div className="p-5 space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div><label className="text-xs font-bold block mb-1">Firma Adı *</label>
                  <input className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:border-orange-400"
                    value={form.name} onChange={e => fld('name', e.target.value)} placeholder="Örn: Güzellik Salonu" /></div>
                <div><label className="text-xs font-bold block mb-1">Kategori *</label>
                  <select className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:border-orange-400"
                    value={form.category} onChange={e => fld('category', e.target.value)}>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><label className="text-xs font-bold block mb-1">Firma Sahibi *</label>
                  <input className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:border-orange-400"
                    value={form.owner_name} onChange={e => fld('owner_name', e.target.value)} placeholder="Ad Soyad" /></div>
                <div><label className="text-xs font-bold block mb-1">Şehir *</label>
                  <input className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:border-orange-400"
                    value={form.city} onChange={e => fld('city', e.target.value)} placeholder="İlçe, Şehir" /></div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div><label className="text-xs font-bold block mb-1">E-posta</label>
                  <input type="email" className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:border-orange-400"
                    value={form.email} onChange={e => fld('email', e.target.value)} placeholder="firma@email.com" /></div>
                <div><label className="text-xs font-bold block mb-1">Telefon</label>
                  <input className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:border-orange-400"
                    value={form.phone} onChange={e => fld('phone', e.target.value)} placeholder="+90 555 000 00 00" /></div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div><label className="text-xs font-bold block mb-1">Başlangıç Fiyatı (₺)</label>
                  <input type="number" className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:border-orange-400"
                    value={form.price_from} onChange={e => fld('price_from', +e.target.value)} /></div>
                <div><label className="text-xs font-bold block mb-1">Plan</label>
                  <select className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:border-orange-400"
                    value={form.plan} onChange={e => fld('plan', e.target.value)}>
      ...
      <div className="w-56 flex-shrink-0 bg-slate-800 flex flex-col h-screen">
        ...
        <nav className="flex-1 p-2 space-y-0.5 overflow-y-auto">
          {NAV.map(([key, icon, label]) => (
            <button key={key} onClick={() => setView(key)}
              ...
              {key === 'firms' && firms.length > 0 && (
                <span className="ml-auto bg-orange-500 text-white text-xs font-bold px-1.5 py-0.5 rounded-full">{firms.length}</span>
              )}
              {key === 'requests' && reviewFirms.length > 0 && (
                <span className="ml-auto bg-amber-500 text-white text-xs font-bold px-1.5 py-0.5 rounded-full">{reviewFirms.length}</span>
              )}
            </button>
          ))}
        </nav>
        <div className="p-3 border-t border-white/[0.07]">
          <Link href="/" className="flex items-center gap-2.5 px-2 py-2 rounded-lg bg-white/[0.05] hover:bg-white/10 transition-colors">
            ...
            <div><div className="text-xs font-semibold text-white/85">Süper Admin</div><div className="text-xs text-white/30">← Ana Sayfa</div></div>
          </Link>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-5">
          {loading ? (
            <div className="flex items-center justify-center gap-3 text-gray-400 py-20"><Spin /> Supabase'den yükleniyor...</div>
          ) : (
            <>
              {/* DASHBOARD */}
              {view === 'dashboard' && (
                <div>
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                    <KPI label="Toplam Firma" value={firms.length} sub={`${activeFirms.length} aktif`} accent="orange" />
                    <KPI label="Kullanıcı" value={profiles.filter(p => p.role === 'customer').length} sub="↑ %12" accent="green" />
                    <KPI label="Randevu" value={appts.length} sub="↑ %8" accent="blue" />
                    <KPI label="Bekleyen" value={reviewFirms.length} accent="red" />
                  </div>
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
                    <div className="lg:col-span-2 bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
                      ...
                      <table className="w-full">
                        ...
                        <tbody>
                          ...
                          {firms.length === 0 && <tr><td colSpan="4" className="px-4 py-8 text-center text-gray-400">Firma yok</td></tr>}
                        </tbody>
                      </table>
                    </div>
                    <div className="space-y-4">
                      <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
                        ...
                        {reviewFirms.length > 4 && (
                          <button onClick={() => setView('requests')} className="w-full text-xs text-orange-500 mt-2 hover:underline">+{reviewFirms.length - 4} daha →</button>
                        )}
                      </div>
                      <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
                ...
                <div>
                  <div className="flex items-center justify-between mb-5">
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => setView('requests')} className="border border-gray-200 bg-white hover:bg-gray-50 text-sm font-semibold px-3 py-1.5 rounded-lg flex items-center gap-1.5 transition-colors">
                        📬 Başvurular
                      ...
                      <button onClick={() => setModal(true)} className="bg-orange-500 hover:bg-orange-600 text-white text-xs font-bold px-4 py-2 rounded-lg transition-colors">+ Firma Ekle</button>
                    </div>
                  </div>
                  <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
                    <div className="p-4 border-b border-gray-100 flex gap-3 flex-wrap">
                      <input className="flex-1 min-w-48 px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:border-orange-400"
                        placeholder="Firma adı, şehir veya e-posta ara..." value={search} onChange={e => setSearch(e.target.value)} />
                      <select className="px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:border-orange-400"
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full">
                        <thead className="bg-gray-50"><tr>
                          {['Firma','Şehir','Kategori','Plan','Durum','Kayıt',''].map(h => (
                        ...
                        <tbody>
                            ...
                            <tr key={f.id} className="border-t border-gray-100 hover:bg-gray-50 group">
                              ...
                              <td className="px-4 py-3">
                                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                  {f.status === 'review' && (
                                    <button onClick={() => approveFirm(f.id, f.name)} className="text-xs bg-orange-500 hover:bg-orange-600 text-white px-2.5 py-1 rounded-lg font-semibold">Onayla</button>
                                  )}
                                  {f.status === 'active' && (
                                    <button onClick={() => suspendFirm(f.id, f.name)} className="text-xs bg-red-50 text-red-600 border border-red-200 hover:bg-red-100 px-2.5 py-1 rounded-lg font-semibold">Askıya Al</button>
                                  )}
                                </div>
                              </td>
                            </tr>
                          ...
                          {filteredFirms.length === 0 && (
                            <tr><td colSpan="7" className="px-4 py-10 text-center text-gray-400">Sonuç bulunamadı</td></tr>
                          )}
                        </tbody>
                      </table>
                    </div>
                ...
                <div>
                  ...
                  {reviewFirms.length > 0 && (
                    <div className="flex items-center gap-3 p-4 bg-amber-50 border border-amber-200 rounded-xl mb-5 text-sm">
                      <span>⚠️</span>
                      <div><b>{reviewFirms.length} başvuru</b> inceleme bekliyor. Onaylandıktan sonra firma aktif olur.</div>
                    </div>
                  )}
                  {reviewFirms.length === 0 ? (
                    ...
                    <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
                      <table className="w-full">
                        <thead className="bg-gray-50"><tr>
                          {['Firma','Sahip','Kategori','Şehir','Başvuru Tarihi','İşlemler'].map(h => (
                        ...
                        <tbody>
                          {reviewFirms.map((f, i) => (
                            <tr key={f.id} className="border-t border-gray-100 hover:bg-gray-50">
                              ...
                              <td className="px-4 py-3">
                                <div className="flex gap-2">
                                  <button onClick={() => approveFirm(f.id, f.name)} className="text-xs bg-orange-500 hover:bg-orange-600 text-white px-3 py-1.5 rounded-lg font-semibold transition-colors">✓ Onayla</button>
                                  <button onClick={() => rejectFirm(f.id, f.name)} className="text-xs bg-red-50 text-red-600 border border-red-200 hover:bg-red-100 px-3 py-1.5 rounded-lg font-semibold transition-colors">✗ Reddet</button>
                                </div>
                              </td>
                            </tr>
                          ))}
              ...
              {view === 'users' && (
                <div>
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-5">
                    <KPI label="Toplam" value={profiles.length} accent="blue" />
                    <KPI label="Müşteri" value={profiles.filter(p => p.role === 'customer').length} accent="green" />
                    <KPI label="Firma Sahibi" value={profiles.filter(p => p.role === 'business_owner').length} accent="orange" />
                    <KPI label="Admin" value={profiles.filter(p => p.role === 'admin').length} accent="gray" />
                  </div>
                  <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
                    <table className="w-full">
                      <thead className="bg-gray-50"><tr>
                        {['Kullanıcı','Telefon','Rol','Puan','Üyelik','Kayıt'].map(h => (
                          <th key={h} className="px-4 py-2.5 text-left text-xs font-bold text-gray-500 uppercase">{h}</th>
                        ))}
                      </tr></thead>
                      <tbody>
                          ...
                          <tr key={p.id} className="border-t border-gray-100 hover:bg-gray-50">
                            ...
                            <td className="px-4 py-3">
                              <span className={`text-xs font-bold px-2 py-0.5 rounded-full border ${
                                p.role === 'admin' ? 'bg-slate-800 text-white border-slate-700' :
                                p.role === 'business_owner' ? 'bg-orange-50 text-orange-600 border-orange-200' :
                            ...
                            <td className="px-4 py-3">
                              <span className="text-xs font-bold px-2 py-0.5 rounded-full border bg-amber-50 text-amber-700 border-amber-200 capitalize">{p.loyalty_tier}</span>
                            </td>
                            <td className="px-4 py-3 text-xs text-gray-400">{new Date(p.created_at).toLocaleDateString('tr-TR')}</td>
                          </tr>
                        ))}
                        {profiles.length === 0 && <tr><td colSpan="6" className="px-4 py-10 text-center text-gray-400">Kullanıcı yok</td></tr>}
                      </tbody>
                    </table>
                  </div>
                </div>
              ...
              {view === 'subscriptions' && (
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-4 mb-5">
                    <KPI label="Pro Plan" value={planCounts.pro || 0} sub={`₺${(planCounts.pro || 0) * 300}/ay`} accent="orange" />
                    <KPI label="Enterprise" value={planCounts.enterprise || 0} sub={`₺${(planCounts.enterprise || 0) * 750}/ay`} accent="blue" />
                    <KPI label="Ücretsiz" value={planCounts.free || 0} sub="Deneme süresi" accent="gray" />
                  </div>
                  <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
                    <table className="w-full">
                      <thead className="bg-gray-50"><tr>
                        {['Firma','Plan','Durum','İşlem'].map(h => (
                          <th key={h} className="px-4 py-2.5 text-left text-xs font-bold text-gray-500 uppercase">{h}</th>
                        ))}
                      </tr></thead>
                      <tbody>
                        {firms.map(f => (
                          <tr key={f.id} className="border-t border-gray-100 hover:bg-gray-50">
                            ...
                            <td className="px-4 py-3">
                              <div className="flex gap-2">
                                {f.plan !== 'enterprise' && (
                                  <button className="text-xs bg-orange-500 hover:bg-orange-600 text-white px-2.5 py-1 rounded-lg font-semibold">Yükselt</button>
                                )}
                        ...
                        {firms.length === 0 && <tr><td colSpan="4" className="px-4 py-10 text-center text-gray-400">Firma yok</td></tr>}
                      </tbody>
                    </table>
                  </div>
                </div>

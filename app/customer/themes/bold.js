...
export default function BoldTheme(props) {
  ...
  return (
    <div className="min-h-screen" style={{background:PAPER,color:INK,fontFamily:SANS,fontFeatureSettings:'"ss01","cv11","kern"'}}>
      ...
      <BusinessDetailModal biz={detailBiz} bizIdx={businesses.findIndex(b=>b.id===detailBiz?.id)} services={bizServices} staff={bizStaff} loading={detailLoading} onClose={()=>setDetailBiz(null)} onBook={()=>setBookModal(true)} variant="bold" uiLang={uiLang}/>
      <BookingModal biz={bookModal&&detailBiz?detailBiz:null} services={bizServices} staff={bizStaff} onClose={()=>setBookModal(false)} onBook={saveBooking} toast3={toast3} paymentEnabled={paymentEnabled} discount={activeAdDiscount} variant="bold" uiLang={uiLang} userId={user?.id} userPoints={user?.loyalty_points||0}/>

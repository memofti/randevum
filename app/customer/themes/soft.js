...
export default function SoftTheme({ user, businesses, appointments, activeAds, profile, tab, setTab, openDetail, detailBiz, bizServices, bizStaff, detailLoading, bookModal, setBookModal, setDetailBiz, activeAdDiscount, paymentEnabled, toast3, userLoc, searchQ, setSearchQ, catFilter, setCatFilter, sortBy, setSortBy, cancelAppt, upcomingAppts, pastAppts }) {
    ...
    <div className="min-h-screen" style={{background:'#fdf6f0'}}>
      ...
      {tab === 'appts' && (
        <div className="max-w-2xl mx-auto px-6 py-8">
          <h1 className="text-2xl font-black mb-6" style={{color:'#6b3a3a'}}>🌸 Randevularım</h1>
          <div className="space-y-3">
            {[...upcomingAppts,...pastAppts].map(a=>(
              <div key={a.id} className="rounded-2xl p-4 flex items-center gap-4" style={{background:'#fff',border:'1.5px solid #fde8d8'}}>
                <div className="w-11 h-11 rounded-full flex items-center justify-center text-xl" style={{background:'#fff5ef'}}>{a.businesses?.emoji||'🏢'}</div>
                <div className="flex-1">
                  <div className="font-bold" style={{color:'#6b3a3a'}}>{a.businesses?.name}</div>
                  <div className="text-xs" style={{color:'#c9a0a0'}}>{a.services?.name} · {new Date(a.appointment_date).toLocaleDateString('tr-TR')} {String(a.appointment_time).slice(0,5)}</div>
                </div>
                <div className="text-xs font-bold px-3 py-1 rounded-full" style={{background:'#fff5ef',color:'#ff8fab'}}>{a.status==='confirmed'?'Onaylı':a.status==='pending'?'Bekliyor':'Tamamlandı'}</div>
              </div>
            ))}
            {upcomingAppts.length===0&&pastAppts.length===0&&<div className="text-center py-16" style={{color:'#e0b0b0'}}>🌸 Henüz randevu yok</div>}
          </div>
        </div>
      )}
      {tab === 'profile' && (
        <div className="max-w-md mx-auto px-6 py-8 text-center">
          <div className="w-20 h-20 rounded-full flex items-center justify-center text-3xl font-black mx-auto mb-4 text-white" style={{background:'#ff8fab'}}>{user.name?.[0]||'?'}</div>
          <div className="text-2xl font-black mb-1" style={{color:'#6b3a3a'}}>{user.name}</div>
          <div className="text-sm mb-6" style={{color:'#c9a0a0'}}>{user.email}</div>
          <button onClick={async()=>{ await supabase.auth.signOut(); localStorage.removeItem('randevu_user'); window.location.href='/login' }}
            className="w-full py-3 rounded-2xl text-sm font-bold" style={{background:'#fff5ef',color:'#c9a0a0',border:'1.5px solid #fde8d8'}}>🚪 Çıkış Yap</button>
        </div>
      )}

...
// Tüm temalardan kullanılan tek logout akışı: supabase oturumu kapat, localStorage temizle, /login'e götür
// onClick={logout} kullanımı React event objesini argüman olarak geçirebilir; defensive olarak string'e zorla
export async function logout(redirect = '/login') {
  const target = typeof redirect === 'string' && redirect.startsWith('/') ? redirect : '/login'
  try { await supabase.auth.signOut() } catch {}
  try { localStorage.removeItem('randevu_user') } catch {}
  if (typeof window !== 'undefined') window.location.href = target
}

// localStorage'daki kullanıcıyı Supabase session ile doğrula — eşleşmiyorsa sil
export async function getActiveUser() {
  ...
  try {
    ...
    if (!session) { localStorage.removeItem('randevu_user'); return null }
    const u = JSON.parse(raw)
    if (u?.id && u.id !== session.user.id) { localStorage.removeItem('randevu_user'); return null }
    return u
  } catch {
    return null
  }
